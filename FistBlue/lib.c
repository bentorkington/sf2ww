...
static int sub_252a(int coord, int greatercoord) {
	// coord has been <<8'd
	int d0 = 0;
	int i;
	for (i=31; i >= 0; --i) {
		coord += coord;
		d0 += d0;
		if ((unsigned int)d0 >= greatercoord) {
			d0 -= greatercoord;
			++coord;
		}
	}
...
u8 calc_flightpath(Object *obj, int x, int y) {		// 24d2 flightpath
	...
	if ((ABS(y-obj->YPI) - ABS(x-obj->XPI)) == 0) {
...
inline static void _init_difficulty(void) { 
	...
	const static char data_2e4e[32]={
		...
		0x04, 0x04, 0x04, 0x04, 0x05, 0x05, 0x05, 0x05, 0x05, 0x06, 0x06, 0x06, 0x06, 0x07, 0x07, 0x08, 
	};
...
static void sub_52bc(u16 *gp, short x, short y, char c) {
	OBJECT_DRAW_NOATTR(gp, x, y, (c & 0xf) + 0x8100);	
